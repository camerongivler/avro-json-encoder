 ...
 * <p>
 * This class is based on https://github.com/Celos/avro-json-encoder/
 * </p>
 * findField method is the only method changed from the original implementation. That method has been updated to handle UNIONs.
 **/
...
public class ExtendedJsonDecoder extends ParsingDecoder
    ...
    public int readIndex() throws IOException {
        ...
        if (currentToken == JsonToken.VALUE_NULL) {
            label = "null";
        } else if (currentToken == JsonToken.START_OBJECT && in.nextToken() == JsonToken.FIELD_NAME) {
            label = in.getText();
            in.nextToken();
    ...
    private JsonParser makeParser(final List<JsonElement> elements) throws IOException {
        return new JsonParser() {
            ...
            @Override public String getValueAsString(String def) throws IOException {
                return null;
            }
    ...
    private void injectDefaultValueIfAvailable(final JsonParser in, String fieldName) throws IOException {
        ...
        JsonNode defVal = JacksonUtils.toJsonNode(field.defaultVal());
        if (defVal == null) {
            throw new AvroTypeException("Expected field name not found: " + fieldName);
        }
    ...
    /**
     * This is the only method changed from https://github.com/Celos/avro-json-encoder/ implementation. Specifically, this method was
     * updated to handle UNIONs.
     */
